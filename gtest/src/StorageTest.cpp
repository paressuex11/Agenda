...
class StorageTest : public ::testing::Test {
  //  Use static variables to hold the configuration for the entire StorageTest
  //  case After each test, TearDown() method will be invoked automatically and
  //  set the the storage to default
 protected:
  static void TearDownTestCase() { recFiles(); }
  virtual void SetUp() {
  ...
  static bool getAllUser(const User &user) { return true; }
  static bool getAllMeeting(const Meeting &meeting) { return true; }
  //  sim*List are taken as a reference to the list in storage
  list<User> simUserList;
  list<Meeting> simMeetingList;
  shared_ptr<Storage> storage;
  //  user1-user3 and meeting1, meeting2 are default configuration for the
  //  test(written in users.csv)
  static const User user1, user2, user3, user4;
  static const Meeting meeting1, meeting2, meeting3;
};

const User StorageTest::user1("Lara Croft", "TombRaidar", "lara@email.com",
                              "13800000000");
const User StorageTest::user2("Geralt of Rivia", "TheWithcer",
                              "geralt@email.com", "13700000000");
const User StorageTest::user3("Naked Snake", "MetalGearSolid",
                              "snake@email.com", "13600000000");
const User StorageTest::user4("Trevor", "GrandTheftAutoV", "Trevor@email.com",
                              "13500000000");
const Meeting StorageTest::meeting1("Naked Snake",
                                    vector<string>({"Lara Croft"}),
                                    Date::stringToDate("2016-07-08/11:10"),
                                    Date::stringToDate("2016-07-08/12:05"),
                                    "I wanna Quite");
const Meeting StorageTest::meeting2(
...
const Meeting StorageTest::meeting3("Lara Croft",
...
TEST_F(StorageTest, MeetingOperation) {
  ...
  auto filter = [&](const Meeting &meeting) {
    return judgeMeetingEqual(meeting, meeting2) ||
           judgeMeetingEqual(meeting, meeting3);
  };
...
class StoragePrivateTest : public StorageTest {
 public:
  virtual void TearDown() { recFiles(); }
};
...
TEST_F(StoragePrivateTest, WriteToFile) {
  ...
  std::vector<std::string> stdUsers{
      R"("Lara Croft","TombRaidar","lara@email.com","13800000000")",
