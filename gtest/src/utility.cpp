...
bool utility::judgeMeetingEqual(const Meeting &meeting1,
                                const Meeting &meeting2) {
...
bool utility::testUserList(const list<User> &expectList,
                           const list<User> &actualList) {
...
bool utility::testMeetingList(const list<Meeting> &expectList,
  ...
  for (auto &meeting : expectList) {
    ...
    EXPECT_TRUE(flag) << "Meeting: " << meeting.getTitle() << " sponsored by "
                      << meeting.getSponsor() << " is not found";
    if (!flag) return false;
  }
  return true;
}
...
void utility::recFiles() {
  ...
  vector<string> meetings{
      R"("Naked Snake","Lara Croft","2016-07-08/11:10","2016-07-08/12:05","I wanna Quite")",
